...
/**
 * The WebAuthn ceremonies for an administrator's passkey: pairing a new key and signing in
...
{
    private const SESSION_REGISTER = 'calmfox_admin_two_factor.passkey.register';

    private const SESSION_LOGIN = 'calmfox_admin_two_factor.passkey.login';
    ...
    public function registrationOptions(TwoFactorAdminUserInterface $adminUser, Request $request): array
    {
        $webAuthn = $this->webAuthn($request);
        $email = (string) $adminUser->getEmail();

        $args = self::publicKey($webAuthn->getCreateArgs(
            self::userHandle($adminUser),
            $email,
            trim(sprintf('%s %s', $adminUser->getFirstName(), $adminUser->getLastName())) ?: $email,
            self::TIMEOUT,
            'preferred',
            true,
            null,
            // the same device must not pair a second key with the same account
            $this->credentialIds($adminUser),
        ));
        // no attestation: which device model the administrator uses is none of our business
     ...
     */
    public function register(TwoFactorAdminUserInterface $adminUser, Request $request, array $response, string $name): void
    {
        ...
        $credentials = $adminUser->getPasskeyCredentials();
        ...
        $adminUser->setPasskeyCredentials($credentials);
    }

    /** @return array<string, mixed> publicKey options for navigator.credentials.get(), limited to this administrator's keys */
    public function loginOptions(TwoFactorAdminUserInterface $adminUser, Request $request): array
    {
        $webAuthn = $this->webAuthn($request);
    ...
    /**
     * Verifies the response of navigator.credentials.get() against this administrator's keys.
     ...
     * @throws PasskeyException
     */
    public function verifyLogin(TwoFactorAdminUserInterface $adminUser, Request $request, array $response): void
    {
        ...
        if (is_string($userHandle) && '' !== $userHandle && self::decode($userHandle) !== self::userHandle($adminUser)) {
            throw PasskeyException::rejected();
        }
    ...
    private function credentialIds(TwoFactorAdminUserInterface $adminUser): array
    {
        return array_map(static fn (array $credential): string => self::decode($credential['id']), $adminUser->getPasskeyCredentials());
    }
    ...
    private static function userHandle(TwoFactorAdminUserInterface $adminUser): string
    {
        $id = $adminUser->getId();
