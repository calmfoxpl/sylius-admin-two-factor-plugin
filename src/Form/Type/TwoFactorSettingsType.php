...
final class TwoFactorSettingsType extends AbstractType
{
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        $builder->add('policy', ChoiceType::class, [
            'label' => 'calmfox_admin_two_factor.settings.policy',
            'expanded' => true,
            'choices' => array_combine(
                array_map(static fn (string $policy): string => 'calmfox_admin_two_factor.settings.policy_' . $policy, TwoFactorPolicy::ALL),
                TwoFactorPolicy::ALL,
            ),
        ]);
    }
    ...
    {
        return 'calmfox_admin_two_factor_settings';
    }
}
