...
namespace Tests\Calmfox\SyliusAdminTwoFactorPlugin\Unit\Passkey;
...
final class PasskeyCeremonyTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->request = Request::create(self::ORIGIN . '/admin/two-factor/setup');
        $this->request->setSession(new Session(new MockArraySessionStorage()));

        $this->adminUser = new AdminUser();
        $this->adminUser->setEmail('admin@example.com');
        (new \ReflectionProperty($this->adminUser, 'id'))->setValue($this->adminUser, 42);
    }
    ...
    public function testItPairsAPasskeyAndLogsInWithIt(): void
    {
        ...
        $options = $this->ceremony->registrationOptions($this->adminUser, $this->request);
        ...
        $this->ceremony->register($this->adminUser, $this->request, $authenticator->register($options, self::ORIGIN), 'Laptop');

        $credentials = $this->adminUser->getPasskeyCredentials();
        ...
        $loginOptions = $this->ceremony->loginOptions($this->adminUser, $this->request);
        self::assertIsArray($loginOptions['allowCredentials'] ?? null);
        self::assertCount(1, $loginOptions['allowCredentials']);

        $this->ceremony->verifyLogin($this->adminUser, $this->request, $authenticator->login($loginOptions, self::ORIGIN, userHandle: '42'));

        self::assertNotNull($this->adminUser->getPasskeyCredentials()[0]['lastUsedAt']);
    }
    ...
    public function testItRejectsAPairingSignedForAnotherOrigin(string $origin): void
    {
        ...
        $this->ceremony->register($this->adminUser, $this->request, (new SoftwareAuthenticator('shop.example.com'))->register($options, $origin), 'Laptop');
    }
    ...
    public function testASubdomainOfTheRelyingPartyIsAccepted(): void
    {
        ...
        $this->ceremony->register($this->adminUser, $this->request, (new SoftwareAuthenticator('shop.example.com'))->register($options, 'https://admin.shop.example.com'), 'Laptop');

        self::assertTrue($this->adminUser->hasPasskeys());
    }

    public function testItRequiresUserVerification(): void
    {
        $authenticator = $this->paired();
        $options = $this->ceremony->loginOptions($this->adminUser, $this->request);

        $this->expectException(PasskeyException::class);

        $this->ceremony->verifyLogin($this->adminUser, $this->request, $authenticator->login($options, self::ORIGIN, SoftwareAuthenticator::USER_PRESENT));
    }
    ...
    public function testAKeyOfAnotherAccountDoesNotLogIn(): void
    {
        ...
        $this->ceremony->verifyLogin($this->adminUser, $this->request, (new SoftwareAuthenticator('shop.example.com'))->login($options, self::ORIGIN));
    }
    ...
    public function testAUserHandleOfAnotherAccountIsRejected(): void
    {
        ...
        $this->ceremony->verifyLogin($this->adminUser, $this->request, $authenticator->login($options, self::ORIGIN, userHandle: '7'));
    }
    ...
    public function testAForgedSignatureIsRejected(): void
    {
        ...
        $this->ceremony->verifyLogin($this->adminUser, $this->request, $response);
    }
    ...
    public function testAFailedAttemptBurnsTheChallenge(): void
    {
        ...
        try {
            $this->ceremony->verifyLogin($this->adminUser, $this->request, $authenticator->login($options, 'https://attacker.example'));
        ...
        $this->ceremony->verifyLogin($this->adminUser, $this->request, $authenticator->login($options, self::ORIGIN));
    }
    ...
    public function testTheSameDeviceCannotBePairedTwice(): void
    {
        ...
        $this->ceremony->register($this->adminUser, $this->request, $authenticator->register($options, self::ORIGIN), 'Again');
    }

    public function testPlainHttpIsAcceptedOnLocalhostOnly(): void
    {
        $request = Request::create('http://localhost/admin/two-factor/setup');
        $request->setSession(new Session(new MockArraySessionStorage()));
        $options = $this->ceremony->registrationOptions($this->adminUser, $request);

        $this->ceremony->register($this->adminUser, $request, (new SoftwareAuthenticator('localhost'))->register($options, 'http://localhost'), 'Dev');
    ...
    public function testAConfiguredRelyingPartyIdWinsOverTheRequestHost(): void
    {
        ...
        $options = $ceremony->registrationOptions($this->adminUser, $this->request);
